...
function CreateZoomForm({ id, setSchedulePop }) {
  ...
  const checkZoomAvailablity = async (e) => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/ZoomLinks/validationOfZoomLinkApi?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const createZoomLink = async (e) => {
    ...
    console.log(form);
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/ZoomLinks/createZoomLink?access_token=${userData?.accessToken}`,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
