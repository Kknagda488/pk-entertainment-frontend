...
function Assesment({ batchId, batchData }) {
  ...
  const getTemplateData = () => {
    fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/templateQuestions/listofTranieeBatchTemplate?filter=%7B%22limit%22%3A%22%22,%22start%22%3A%22%22,%22search%22%3A%22%22%7D&access_token=${userData?.accessToken}`
    )
      .then(async (response) => {
        const data = await response.json();
  ...
  const feedback = async (questionObject) => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/Assessments/addQuestionBunchInTranieeBatch?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const handleDeleteFormBatchQuestion = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/FormBatchquestions/delete/${id}?access_token=${userData.accessToken}`,
        {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
          },
  ...
  const addAssessments = async (e) => {
    ...
    console.log(formData);
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/Assessments/add?access_token=${userData?.accessToken}`,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
            ...
            <div className="relative mx-20 flex flex-col  w-1/2 items-center justify-center bg-white rounded-lg shadow-md px-8 py-2">
              ...
              >
                <DeletePopupCard
                  ...
                  confirmDelete={() => handleDeleteFormBatchQuestion(selectedRow.id)}
                />
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col w-full justify-between ">
                <div className="flex justify-between  w-full">
                  ...
                  <div className="flex w-fit justify-end space-x-2">
                    <Link to="/questionbanks" target="_blank" rel="noopener noreferrer">
