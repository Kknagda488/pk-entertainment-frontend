...
const Attendance = ({ batchId, batchData }) => {
  ...
  const fetchAttendence = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batchAttendences/list/${batchId}?filter=${params}&access_token=${userData?.accessToken}`
      );
  ...
  const exportAttendanceReport = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batchAttendences/export/${batchId}?access_token=${userData?.accessToken}`
      );
      // check for error response
      if (!response.ok) {
        // get error message from body or default to response statusText
  ...
  const handleAllExport = (allExportAttendenceData) => {
    ...
    Object.entries(allExportAttendenceData).forEach(entry => {
      ...
      value.forEach((data, index) => {
        ...
        employees.push([
          data.firstName ? `${data.firstName} ${data.lastName || ''}` : `${data.firstName} ${data.lastName || ''}`,
    ...
    Utils.exportAoaToXlsx(employees, `${batchData.name} - attendance-${Date.now()}`);
  };
  ...
  const handleExport = () => {
    ...
    attendence.forEach((data, index) => {
      employees.push([
        index + 1,
        data.nomineeAttendance
          .email,
        data.nomineeAttendance
          .firstName + " " + data.nomineeAttendance
          .lastName,
            ...
            return (
              <tbody key={uuid_v4()}>
                <tr className="bg-white border-b  ">
                  ...
                  <td className="px-6 py-4 text-gray-900">{moment(data?.attendenceDate).format("DD-MM-YYYY")}</td>
