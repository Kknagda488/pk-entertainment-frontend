  ...
  const fetchBatchPeopleList = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/nominees/list/batch/${batchId}?filter=${params}&access_token=${userData?.accessToken}`
      );
              ...
              userData?.client === "kotak" ? (
                ...
                <>
                  <tr>
                    ...
                    {userData?.client === "axis" ? (<th scope="col" className="px-6 py-3">
                      Employee Id
                    </th>) : null}
              ...
              <tbody key={uuid_v4()}>
                {
                  userData?.client === "kotak" ? (
                    ...
                    /> */}
                      ...
                      <td className="px-6 py-4 text-gray-900">{data.employeeId}</td>


                      <td className="px-6 py-4 text-gray-900">{
                        new Date(batchData.batchStartDate).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })
                      }</td>

                      <td className="px-6 py-4 text-gray-900">{data.referenceCode}</td>
                      <td className="px-6 py-4 text-gray-900">{data.branchName}</td>
