...
const EditTemplate = ({ templateId, setShowEditTemplate }) => {
  ...
  const fetchTemplateById = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/emailtemplate/getTemplateById/${id}?access_token=${userData?.accessToken}`
      );
  ...
  const handleUpdateTemplate = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/emailtemplate/update/${templateId}?access_token=${userData?.accessToken}`,
        {
          method: "PUT",
          headers: {
