...
const EmailLog = ({ batchId, batchData }) => {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/emailtemplate/all/${batchData?.programId}?access_token=${userData?.accessToken}`
      );
  ...
  const sendMail = async (e) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/emailtemplate/sendmail/${templateId}?access_token=${userData?.accessToken}`,
      {
        body: form,
        headers: {
