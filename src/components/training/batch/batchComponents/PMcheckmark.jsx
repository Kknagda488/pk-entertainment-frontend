...
function PMcheckmark({ batchData, batchId, getOneBatch }) {
  ...
  async function addPmCheckmark(e) {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/checkmarks/add?access_token=${userData?.accessToken}`,
        {
          headers: {
  ...
  async function fetchPmCheckmarks() {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/checkmarks/list/${batchId}?access_token=${userData?.accessToken}`
      );
      const data = await response.json();
      if (!response.ok) {
        toast.error(data?.error?.message);
...
const CheckMarkRow = ({ index, item, fetchPmCheckmarks }) => {
  ...
  const handleEditCheckmark = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/checkmarks/updateStatus/${item.id}?access_token=${userData?.accessToken}`,
        {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
          },
