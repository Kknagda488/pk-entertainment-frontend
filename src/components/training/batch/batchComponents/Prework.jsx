...
const Prework = ({ batchId, batchData }) => {
  ...
  async function handlePreworkUpdate(e) {
    ...
    try {
      ...
      let response = await fetch(`${process.env.REACT_APP_PUBLIC_URL}/api/preworks/uploadFile/${batchId}?access_token=${userData?.accessToken}`, {
        headers: {
  ...
  const handleSendMail = async (files, e) => {
    ...
    try {
      ...
      response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batches/sendPreworkMails?access_token=${userData?.accessToken}`,
        {
          headers: {
  ...
  const fetchPrework = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/preworks/listByBatch/${batchId}?access_token=${userData?.accessToken}`
      );
      const data = await response.json();
      if (!response.ok) {
        throw data.message;
