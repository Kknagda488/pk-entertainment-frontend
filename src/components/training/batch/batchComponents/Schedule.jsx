...
function Schedule({ setSchedulePop, schedulePop, id, batchData, openTab }) {
  ...
  var link = `${process.env.REACT_APP_PUBLIC_LINK}/livebatchtracking/batch&id=${id}&batch&name=${batchData.name}&trackingid=${id}`;
  ...
  const getZoomLinks = () => {
    fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/ZoomLinks/traineeBatchzoomLinkByBatchId?id=${id}&filter=%7B%22limit%22%3A%22%22,%22start%22%3A%22%22,%22search%22%3A%22%22%7D&access_token=${userData?.accessToken}`
    )
      .then(async (response) => {
  ...
  const getAssessmentLinks = () => {
    fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/batchAssessments/traineeBatchAssessmentdetailsByBatchId?id=${id}&filter=%7B%22limit%22%3A%22%22,%22start%22%3A%22%22,%22search%22%3A%22%22%7D&access_token=${userData?.accessToken}`
    )
      .then(async (response) => {
        const data = await response.json();
  ...
  const sendEmail = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/batches/sendEmailToTrainer?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const sendZoomEmail = async () => {
    ...
    let form = new URLSearchParams(Object.entries(formData)).toString();
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/userBatchAnswers/sendZoomLinkEmailToTrainer?access_token=${userData?.accessToken}`,
      {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
  ...
  return (
    <div className="flex flex-col">
      ...
      <motion.div {...animations} animate={{ height: !openZoom ? 0 : "25rem" }}>
        <ul
          id="journal-scroll"
          className={`${openZoom ? "overflow-y-auto h-[25rem]" : " h-0 overflow-hidden"
      ...
      <motion.div {...animations} animate={{ height: !open ? 0 : "25rem" }}>
        <ul
          id="journal-scroll"
          className={`${open ? "overflow-y-auto h-[25rem]" : " h-0 overflow-hidden"
            }  px-3   text-sm text-gray-700  ease-in-out transition-all`}
        >
          <div className="mt-4 flex w-full  flex-col  sm:flex-row ">
            <div className="grow rounded border-2  border-gray-300 py-2 px-3 focus:border-emerald-500 focus:outline-none sm:rounded-l-md sm:rounded-r-none sm:border-r-0">
              ...
              <div className="flex w-full justify-between items-center">
                <div className="flex flex-col">
                  <h1 className="text-lg font-medium w-[25rem]">
                    ...
                    <button
                      onClick={handleMailSelect}
                      className={`text-sm ${assessmentList.includes("attendance")
                  ...
                  <span className="flex flex-col w-full items-start">
                    <h1 className="text-md  text-gray-600 ">
                      <a
                        href={`${process.env.REACT_APP_PUBLIC_LINK}/attendance-form/${batchData.batchId}?programType=${batchData?.program?.programType}&client=${userData?.client}&key=${userData?.tenantKey}`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        {`${process.env.REACT_APP_PUBLIC_LINK}/attendance-form/${batchData.batchId}?programType=${batchData?.program?.programType}`}
                      </a>
                    </h1>
                  </span>
                </div>
