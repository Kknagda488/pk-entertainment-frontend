...
function ScheduledAssessments({
  ...
  const deleteAssessment = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/Assessments/deleteById/${id}?access_token=${userData?.accessToken}`,
        {
          method: "DELETE",
          headers: {
  ...
  const url = `${process.env.REACT_APP_PUBLIC_LINK}/quiz/${id}/${encodeURIComponent(batchData.name)}/${test?.assessmentType}/${test?.assessmentId}`;
  console.log("--------------", url)
