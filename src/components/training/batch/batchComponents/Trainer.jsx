...
function Trainer({ batchId, getOneBatch, batchData }) {
  ...
  async function handleTrainerUpdate(e) {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batches/addTrainer/${batchId}?access_token=${userData?.accessToken}`,
        {
          headers: {
  ...
  const fetchTrainerData = async (value) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/programTrainers/getTrainers?name=${value}&access_token=${userData?.accessToken}`
      );
      const data = await response.json();
