...
function AnnouncementList({ programId }) {
  ...
  const fetchAnnouncementList = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/announcements/list/${programId}?filter=${params}&access_token=${userData?.accessToken}`
      );
  ...
  const handleMailSend = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/announcements/sendMail/${programId}?access_token=${userData?.accessToken}`,
        {
          headers: {
