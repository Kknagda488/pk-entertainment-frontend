...
function CustomQuiz2({ quiz, questionList, id, setType, setResult, setNoOfAttemptsRemain, setError }) {
  ...
  const submitTest = (e) => {
    ...
    const formatedEndtime = new Date().toISOString().split('T')[0] + ' ' + new Date().toTimeString().split(" ")[0];
    const formatedstarttime = startTime.toISOString().split('T')[0] + ' ' + startTime.toTimeString().split(" ")[0];
  ...
  const saveAssessment = async (formData) => {
    ...
    if (!data.value) {
      console.log(data?.data?.noOfAttemptsRemain)
      if (data?.data?.noOfAttemptsRemain <= 0) {
        setType("submitted");
        setError(true)
        setBucket([]);
      } else {
      ...
      // setSubmitted(true);
      setNoOfAttemptsRemain(data.data.noOfAttemptsRemain)
