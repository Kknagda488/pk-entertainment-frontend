...
const MonthlyDeckReports = () => {
  ...
  const reportDownloader = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/reports/auditReport?access_token=${userData?.accessToken}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      if (data.code === 404) {
        ...
        if (data) {
          ...
          handleAuditReport(allExportAttendenceData);
  ...
  const handleAuditReport = (allExportAttendenceData) => {
    let employees = [
      [
        "Emp. Id",
        "Emp Name",
        "First Invite Date",
        "Status",
        "Assessment Score in Absolute value",
        "Assessment Score in %",
        "Second Invite Date",
        "Status",
        "Assessment Score in Absolute value",
    ...
    allExportAttendenceData.forEach((data, index) => {
      let rowData = [
        data?.employeeId || "",
        data?.employeeName || "",
        data?.firstInviteDate || "",
        data?.status || "",
        data?.assessmentScore || "",
        data?.assessmentScorePercentage || "",

        data?.secondInviteDate || "",
        data?.status1 || "",

        data?.assessmentScore1 || "",
        data?.assessmentScorePercentage1 || "",
      ];
    ...
    Utils.exportAoaToXlsx(employees, `Audit-Report-${Date.now()}`);
  };
