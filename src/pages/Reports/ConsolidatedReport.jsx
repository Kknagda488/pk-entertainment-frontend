...
const ConsolidatedReport = () => {
  ...
  const reportDownloader = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/batchAttendences/liveTrackingAttendence?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const handleAllExport = (allExportAttendenceData) => {
    ...
    allExportAttendenceData.forEach((data, index) => {
      ...
      if (userData?.client === "kotak") {
        rowData = [
          ...
          data?.day3 || "A",
          3,
          data?.totalPresentDays || 0,
          data?.learningHours || "0",
          data?.feedbackPoint || "0",
    ...
    // Assuming Utils.exportAoaToXlsx is correctly implemented Generate
    Utils.exportAoaToXlsx(employees, `Consolidated-Report-${Date.now()}`);
  };
