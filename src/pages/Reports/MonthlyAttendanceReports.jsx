...
const MonthlyAttendanceReports = () => {
  ...
  const getProgramData = () => {
    fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/programs/listofProgramName?id=${programId}&filter=%7B%22search%22%3A%22%22%7D&access_token=${userData?.accessToken}`
    )
      .then(async (response) => {
        const data = await response.json();
  ...
  const reportDownloader = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/reports/monthlyAttendanceReport?access_token=${userData?.accessToken}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      if (data.code === 404) {
        ...
        if (data) {
          ...
          toast.success("Reports fetch success")
          monthlyReportHandleAllExportNew(data)
  ...
  const monthlyReportHandleAllExportNew = (allExportAttendenceData) => {

    // Initialize the headers with the first row
    let employees = [
      [
        "Days",
        "Day-1",
    ...
    const {
      batchTotalPresent,
      overallBatchPercentageAttendance,
      BatchperDayAttendancePercentage,
      overallBatchPresentAttendance,
      nomineeCount,
    ...
    // Construct rows
    const row1 = [...batchTotalPresent, overallBatchPresentAttendance];
    const row2 = [...BatchperDayAttendancePercentage, overallBatchPercentageAttendance];
    const row3 = Array(5).fill(nomineeCount);

    row3.push(nomineeCount * 5)

    // Concatenate rows to employees arr  ay
    employees[1] = employees[1].concat(row2);
    employees[2] = employees[2].concat(row1);
    employees[3] = employees[3].concat(row3);

    // Export to XLSX
    Utils.exportAoaToXlsx(employees, `Monthly-Attendance-Report-${Date.now()}`);
    ...
    // for (let i = 0; i < allExportAttendenceData?.noimeeCount?.length; i++) {
    //   employees[1].push(allExportAttendenceData?.perDayAttendancePercentage[i]);

    //   employees[2].push(allExportAttendenceData?.totalPresent[i]);

    //   employees[3].push(allExportAttendenceData?.nomination[i]);
    // }

    // employees[1].push(allExportAttendenceData?.overallPercentageAttendance);

    // employees[2].push(allExportAttendenceData?.overallPresentAttendance);

    // employees[3].push(allExportAttendenceData?.overallNominationAttendance);
  ...
  const reportCreator = (reportData) => {
    ...
    setGenerated({
      filename: `${moment(dates[0].startDate).format("ll")} to ${moment(
        dates[0].endDate
      ).format("ll")}_${selectedClient.label} ${selectedProgram.label}_${selectedType.label
        } Report`,
  ...
  const fetchEmployeeAttendence = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batchAttendences/attendenceReport/${programId}?access_token=${userData?.accessToken}`
      );
      // check for error response
      if (!response.ok) {
        // get error message from body or default to response statusText
  ...
  async function saveAsExcel(data) {
    ...
    const updatedReport = data.map((report) => {
      const updatedReportData = {
        ...
        "Date": report.date,
        "Day": report.day,
        "Average Score(Out of 20)-Pre-Assessment": report.averagePreAssessmentScore,
        "Average Score(Out of 20)-Post Assessment": report.averagePostAssessmentScore,
    ...
    var header =
      ["Program Name", "Batch", "Batch Type", "Time", "Total Nomination/List", "No.of Participants- Day-1", "No.of Participants Day-2", "Trainer Name", "Date", "Day", "Average Score(Out of 20)-Pre-Assessment", "Average Score(Out of 20)-Post Assessment", "Overall Program Rating"]
