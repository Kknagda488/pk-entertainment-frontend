...
const MonthlyDeckReports = () => {
  ...
  const reportDownloader = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/reports/monthlyDeckReport?access_token=${userData?.accessToken}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      if (data.code === 404) {
        ...
        if (data) {
          ...
          handleMonthlyDeskReport(allExportAttendenceData);
  ...
  const handleMonthlyDeskReport = (allExportAttendenceData) => {
    let employees = [
      [
        "Emp. Id",
        "Full Name",
        "Assessment % (More than 60%)",
        "Total Average Feedback",
        "Attendance %",
      ],
    ];
    ...
    allExportAttendenceData.forEach((data, index) => {
      ...
      let rowData = [
        ...
        data?.assessmentScorePercentage || "",
        data?.FeedbackPoint || "",
        data?.totalPresentPercentage || "",
      ];
    ...
    Utils.exportAoaToXlsx(employees, `Monthly-Deck-Report-${Date.now()}`);
  };
...
export default MonthlyDeckReports