...
const SummaryReport = () => {
  ...
  const reportDownloader = async () => {
    ...
    console.log("selectedProgram.data++++++++++++++++++", selectedProgram);
    const formData = {
      ...
      // assessmentType: selectedType.data,
    };
    var response;
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/reports/summaryReport?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const reportCreator = (reportData) => {
    ...
    setGenerated({
      filename: `${moment(dates[0].startDate).format("ll")} to ${moment(
        dates[0].endDate
      ).format("ll")}_${selectedClient.label} ${selectedProgram.programNAme}_${selectedType.label
        } Report`,
  ...
  function handleAllExport(allExportAttendenceData) {
    ...
    allExportAttendenceData.forEach((data, index) => {
      let rowData = [
        ...
        data?.BatchType,
        data?.Time,
        data?.totalNomination,
        data?.noOfParticipantsDay1,
        data?.noOfParticipantsDay2,
        data?.trainerName,
        data?.date,
        data?.day,
        data?.averagePreAssessmentScore,
        data?.averagePostAssessmentScore,
        data?.overallProgramRating,
      ];
    ...
    Utils.exportAoaToXlsx(employees, `Summery-Report-${Date.now()}`);
  };
  ...
  //     totalNominationList: 50,
  //     participantsDay1: 25,
  //     participantsDay2: 20,
  //     trainerName: 'John Doe',
  //     date: '2023-01-10',
  //     day: 'Monday',
  //     avgScorePreAssessment: 15,
  //     avgScorePostAssessment: 18,
  //     overallProgramRating: 'Excellent',
  //   },
  //   {
  //     id: 2,
  ...
  //     totalNominationList: 40,
  //     participantsDay1: 20,
  //     participantsDay2: 18,
  //     trainerName: 'Jane Smith',
  //     date: '2023-01-15',
  //     day: 'Saturday',
  //     avgScorePreAssessment: 14,
  //     avgScorePostAssessment: 17,
  //     overallProgramRating: 'Good',
  //   },
  ...
  //   { field: 'batchType', headerName: 'Batch Type', flex: 1 },
  //   { field: 'time', headerName: 'Time', flex: 1 },
  //   { field: 'totalNominationList', headerName: 'Total Nomination/List', type: 'number', flex: 1 },
  ...
  //   { field: 'avgScorePreAssessment', headerName: 'Avg Score - Pre Assessment', type: 'number', flex: 1 },
  //   { field: 'avgScorePostAssessment', headerName: 'Avg Score - Post Assessment', type: 'number', flex: 1 },
  //   { field: 'overallProgramRating', headerName: 'Overall Program Rating', flex: 1 },
  // ];
