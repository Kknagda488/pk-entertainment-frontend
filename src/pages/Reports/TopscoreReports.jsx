...
const TopscoreReports = () => {
  ...
  const reportDownloader = async () => {
    ...
    response = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/api/reports/topScorerReport?access_token=${userData?.accessToken}`,
      {
        headers: {
  ...
  const handleTopScorerReport = (allExportAttendenceData) => {
    let employees = [
      [
        "Region",
        "Emp. Code",
    ...
    Utils.exportAoaToXlsx(employees, `TopScorer-Report-${Date.now()}`);
  };
...
export default TopscoreReports