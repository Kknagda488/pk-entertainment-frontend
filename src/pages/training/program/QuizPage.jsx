...
function QuizPage() {
  ...
  const getQuestionData = () => {
    fetch(
      ...
      .then(async (response) => {
        ...
        // console.log(a);
        console.log("date", today.isBetween(data.data?.assessmentRes?.startDate, data.data?.assessmentRes?.endDate, null, '[]'))
        setIsBetweentodayStatus(today.isBetween(data.data?.assessmentRes?.startDate, data.data?.assessmentRes?.endDate, null, '[]'))
  ...
  } else if (type == "submitted" && teamData?.assessmentRes?.assessmentType == "custom" && teamData?.assessmentRes?.showScore == 1) {
      ...
      <h5 className="text-3xl font-semibold leading-none text-center text-gray-700 mb-[40px]">
        {
          error ?
            ...
            <>
              {
                ((result?.obtainMarks / result?.totalMarks) * 100).toFixed(2) >= 90 ?
                  <>
      ...
      </h5>
      {
        !error ?
          <div className="grid grid-cols-4 justify-items-center bg-[#f1f8f8]">
            <div className="p-5">
              <p className="font-semibold text-[2rem] text-[#02756d] text-center">{result?.totalMarks}</p>
              <h2 className="text-gray-500 font-semibold text-center">Total Points</h2>
            </div>
            <div className="p-5">
              <p className="font-semibold text-[2rem] text-[#02756d] text-center">{result?.obtainMarks}</p>
              <h2 className="text-gray-500 font-semibold text-center">Your Points</h2>
            </div>
            <div className="p-5">
              <p className="font-semibold text-[2rem] text-[#02756d] text-center">{((result?.obtainMarks / result?.totalMarks) * 100).toFixed(2)}%</p>
              <h2 className="text-gray-500 font-semibold text-center">Percentage score (%)</h2>
            </div>
            <div className="p-5">
              <p className="font-semibold text-[2rem] text-[#02756d] text-center capitalize">{result?.result}</p>
              <h2 className="text-gray-500 font-semibold text-center ">Status</h2>
            </div>
          </div>
          :
          null
