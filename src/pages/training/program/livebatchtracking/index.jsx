...
function BatchReports({ session }) {
  ...
  const fetchAssessmentData = async () => {
    try {
      var response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/userBatchAnswers/all/${batchId}?access_token=${userData?.accessToken}`,
        {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
          },
  ...
  const fetchAssessmnetPeopleList = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_PUBLIC_URL}/api/batchAssessments/listAssessmentofLiveTrackingLinks/${batchId}?filter=${params}&access_token=${userData?.accessToken}`
      );
      const data = await response.json();
  ...
  const updateAssessment = async (item, status) => {
    if (status === "Update") {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_PUBLIC_URL}/api/userBatchAnswers/updateAssessmentData/${item?.userBatchAnswerId}?access_token=${userData?.accessToken}`,
          {
            headers: {
              "Content-Type": "application/x-www-form-urlencoded",
            },
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_PUBLIC_URL}/api/userBatchAnswers/addAssessmentData?access_token=${userData?.accessToken}`,
          {
            headers: {
              "Content-Type": "application/x-www-form-urlencoded",
            },
